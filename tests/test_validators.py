...
class TestLooksLikeGibberish:
    ...
    def test_poor_alphabet_caught(self):
        """«выавываыавыавы» — 64% гласных, но всего три буквы по кругу.

        Пришло из настоящей заявки: правило про согласные тут бессильно,
        поэтому смотрим ещё и на богатство алфавита.
        """
        for value in ("выавываыавыавы", "Ываываывавыа", "Ывавыаываываы",
                      "фывфывфывфыв", "asdasdasdasd"):
            assert looks_like_gibberish(value), value

    def test_judged_word_by_word(self):
        """«Член выавываыавыавы» целиком даёт семь букв и выглядит прилично."""
        assert looks_like_gibberish("Член выавываыавыавы")

    def test_long_real_names_pass(self):
        """У длинного настоящего названия букв всегда много.

        Порог по числу РАЗЛИЧНЫХ букв абсолютный, а не долей: доля с ростом
        длины падает и у осмысленного текста.
        """
        for value in ("ИНДИВИДУАЛЬНЫЙ ПРЕДПРИНИМАТЕЛЬ НИЦЕВИЧ ТАТЬЯНА ВАЛЕРЬЕВНА",
                      "Индивидуальный предприниматель Гагин Евгений Николаевич",
                      "Неисключительная лицензия на программу для ЭВМ",
                      "сопровождение и подготовка материалов для отчета",
                      'ООО "ДС ФУЛФИЛМЕНТ"', "ИП МАМПОРИЯ ВАЛЕРИЯ АЛЕКСЕЕВНА",
                      "ТСЖ «Первомайское»", "Услуги подрядчиков", "Член"):
            assert not looks_like_gibberish(value), value

    def test_digits_are_left_to_the_hard_layer(self):
        """Мягкий слой цифры не судит — иначе ловил бы обычные комментарии.

        «Счёт №101 от 21.08.2026» — одиннадцать цифр против шести букв, и это
        нормальная строка. Значение из ОДНИХ цифр отвергает жёсткий слой.
        """
        for value in ("Договор 1202/2-2026", "15.12.2026", "услуга на 6 месяцев",
                      "аренда за август 2026", "Счёт №101 от 21.08.2026",
                      "оплата на счёт 40802810703500018523"):
            assert not looks_like_gibberish(value), value
        assert looks_broken("12321432132132132131253542365432565324634")
