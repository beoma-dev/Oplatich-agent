...
# Между меткой и значением бывает уточнение в скобках («Поставщик
# (Исполнитель):»), и в PDF-слое оно нередко переносится на другую строку.
# Прежний вариант допускал до значения ЛЮБЫЕ 30 символов — и на строке без
# двоеточия («Исполнитель Адвокат ЗУБЕНКО АНДРЕЙ ИГОРЕВИЧ, 196084…») съедал
# начало имени, оставляя огрызок «ИЧ, 196084, РОССИЯ». Теперь разделитель —
# только пробелы, скобочное уточнение и двоеточие.
...
# Метки ПЛАТЕЛЬЩИКА. Его имя выглядит ровно как контрагент — та же «ИП» или
# «ООО» с ФИО, — но платим не ему. На реальном счёте адвоката запасной путь
# подставлял именно заказчика, то есть нашу же сторону.
_PAYER_RE = re.compile(r"плательщик|покупатель|заказчик", re.I)
...
# Слово имени: «Зубенко» или «ЗУБЕНКО» — в счетах ФИО получателя сплошь
# набрано капслоком, и вариант только с заглавной первой буквой его не видел.
_NAME_WORD = r"[А-ЯЁ][а-яё]+|[А-ЯЁ]{2,}"
# Кем человек выступает получателем, если он не ИП: адвокат, нотариус,
# арбитражный управляющий, самозанятый — форма пишется словом перед ФИО.
_PROFESSION = r"Адвокат|Нотариус|Арбитражный\s+управляющий|Самозанят\w+"
_FIO_START_RE = re.compile(
    rf"^\W*((?:(?:{_PROFESSION})\s+)?"
    rf"(?:{_NAME_WORD})(?:-(?:{_NAME_WORD}))?"
    rf"\s+(?:(?:{_NAME_WORD})\s+(?:{_NAME_WORD})|[А-ЯЁ]\.\s*[А-ЯЁ]\.))"
)
...
def find_counterparty(text: str) -> str | None:
    ...
    # имя стоит СЛЕДУЮЩЕЙ строкой после метки («Плательщик» / «Покупатель» /
    # «(Заказчик):»), поэтому строку после метки плательщика тоже пропускаем.
    payer_above = False
    for line in text.splitlines():
        if not line.strip():
            continue
        is_payer = bool(_PAYER_RE.search(line))
        skip = is_payer or payer_above or _BANK_LINE_RE.search(line)
