"""Уведомления финансистам: карточка каждой заявки.

Карточка уходит по КАЖДОЙ заявке (срочные помечены 🔴), с вложениями:
PDF-документ заявки и сам файл счёта. Под ней одна кнопка — «Открыть
в приложении»: статус ставят в панели Mini App, где рядом видно счёт,
реквизиты и историю. Кнопки «Оплачено/Отложено/Отклонено» остаются лишь
там, где приложение не настроено (см. build_card_keyboard), и на уже
разосланных карточках — их обрабатывает bot/finance_actions.py.
"""
...
async def notify_finance(
    ...
    Карточка — подпись к документу со ссылкой в панель: приложен файл счёта,
