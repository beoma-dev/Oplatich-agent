...
Что в архиве, а чего нет. При `STORAGE_BACKEND=local` — всё: реестр, файлы
счетов, журнал безопасности, настройки. При `google` заявки живут в таблице,
а счета в папке Диска, и в tar.gz **не попадают**: там остаются журнал
безопасности с аудитом, настройки бота и справочник пользователей. Панель
показывает состав честно, по текущему бэкенду (карточка «Бэкап данных»).

Восстановление, когда сервер жив (архив уже лежит в `data/backups/`, копия
из личка для этого не нужна):

```bash
docker compose stop app
tar -xzf data/backups/invoice-bot-backup-ГГГГММДД-ЧЧММСС.tar.gz -C data
sudo chown -R 1000:1000 data
