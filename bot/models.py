...
class InvoiceRequest:
    ...
    # на 6 месяцев». Поле ОБЯЗАТЕЛЬНОЕ во всех каналах подачи (required=True
    # и в api/routes, и в чат-форме); пустая строка по умолчанию — только
    # ради заявок, собранных не из формы (импорт, тесты).
    work_deadline: str = ""
    ...
    # Закрывающие документы: акт, УПД, накладная. Приходят ПОСЛЕ оплаты,
    # иногда через месяц, и дописываются в уже существующую строку реестра —
    # поэтому в модели пусты почти всегда, а заполняются отдельной ручкой.
    closing_files: list[str] = field(default_factory=list)
    ...
    def as_sheet_row(self) -> list[str]:
        ...
        return [
            ...
            "\n".join(self.closing_files),                   # Закрывающие документы
        ]
...
SHEET_HEADERS: list[str] = [
    ...
    # И эта в конец: правило то же — вставка в середину сдвинет заполненные
    # строки. Заполняется не при подаче, а когда документы принесут.
