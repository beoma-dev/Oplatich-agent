...
callback_data: ST:<request_id>:<KEY> (см. services/notifier.build_card_keyboard).

Кнопок этих под НОВЫМИ карточками больше нет — статус ставят из панели
Mini App. Обработчик остаётся ради уже разосланных сообщений: они живут
в переписке месяцами, и нажатие на них не должно упираться в тишину.
Он же работает там, где приложение не настроено вовсе: тогда кнопки
статуса под карточкой остаются единственным способом сменить статус.
