...
/** Человеческий текст для ответа без тела.
 *
 * 413 приходит от прокси ДО приложения и с пустым телом: `r.json()` падает,
 * и на экране оставалось «не удалось» без единой подсказки. Пятнадцать
 * мегабайт скана — обычное дело, и человек должен понимать, что делать.
 */
function httpProblem(status) {
  if (status === 413) {
    return "Файлы слишком велики для одной отправки — пришлите их по частям.";
  }
